...
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Frepo-contents.png" alt="The lakeFS objects list with a highlight to indicate that the branch is set to main." class="quickstart"/>
...
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-main-01.png" alt="The lakeFS object viewer with embedded DuckDB to query parquet files. A query has run automagically to preview the contents of the selected parquet file." class="quickstart"/>
...
FROM     READ_PARQUET('lakefs://firstrepo/main/lakes.parquet')
GROUP BY country
ORDER BY COUNT(*)
DESC LIMIT 5;
```

<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-main-02.png" alt="An embedded DuckDB query showing a count of rows per country in the dataset." class="quickstart"/>
...
docker exec lakefs \
    lakectl branch create \
	    lakefs://firstrepo/denmark-lakes \
      --source lakefs://firstrepo/main
```

_You should get a confirmation message like this:_

```bash
Source ref: lakefs://firstrepo/main
...
From the lakeFS [**Objects** page](/repositories/firstrepo/objects?ref=main) select the [`lakes.parquet`](/repositories/firstrepo/object?ref=main&path=lakes.parquet) file to open the DuckDB editor: 
...
CREATE OR REPLACE TABLE lakes AS 
    SELECT * FROM READ_PARQUET('lakefs://firstrepo/denmark-lakes/lakes.parquet');
```
...
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-editor-02.png" alt="The DuckDB editor pane querying the lakes table" class="quickstart"/>
...
<img src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-editor-03.png" alt="The DuckDB editor pane deleting rows from the lakes table" class="quickstart"/>
...
<img src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-editor-04.png" alt="The DuckDB editor pane querying the lakes table showing only rows for Denmark remain" class="quickstart"/>
...
```sql
COPY lakes TO 'lakefs://firstrepo/denmark-lakes/lakes.parquet';
```

<img src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-editor-05.png" alt="The DuckDB editor pane writing data back to the denmark-lakes branch" class="quickstart"/>
...
SELECT   country, COUNT(*)
FROM     READ_PARQUET('lakefs://firstrepo/denmark-lakes/lakes.parquet')
GROUP BY country
ORDER BY COUNT(*)
DESC LIMIT 5;
```

<img src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-editor-06.png" alt="The DuckDB editor pane show the parquet file on denmark-lakes branch has been changed" class="quickstart"/>
...
DESC LIMIT 5;
```
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-main-02.png" alt="The lakeFS object browser showing DuckDB querying lakes.parquet on the main branch. The results are the same as they were before we made the changes to the denmark-lakes branch, which is as expected." class="quickstart"/>
...
```bash
docker exec lakefs \
  lakectl commit lakefs://firstrepo/denmark-lakes \
...
```bash
Branch: lakefs://firstrepo/denmark-lakes
...
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-main-03.png" alt="The lakeFS object browser with a DuckDB query on lakes.parquet showing that there is only data for Denmark." class="quickstart"/>
...
<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fduckdb-main-02.png" alt="The lakeFS object viewer with DuckDB query showing that the lakes dataset on main branch has been successfully returned to state prior to the merge." class="quickstart"/>
...
1. Go to the **Uncommitted Changes** tab in the UI, and make sure that you see the new file in the path shown: 
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-00.png" alt="lakeFS Uncommitted Changes view showing a file called `check_commit_metadata.yml` under the path `_lakefs_actions/`" class="quickstart"/>
...
1. Now we'll merge this new branch into `main`. From the **Compare** tab in the UI compare the `main` branch with `add_action` and click **Merge**
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-01.png" alt="lakeFS Compare view showing the difference between `main` and `add_action` branches" class="quickstart"/>
...
1. Create a new branch (see above instructions on how to do this if necessary) called `etl_20230504`. Make sure you use `main` as the source branch. 
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-02.png" alt="lakeFS branch etl_20230504 with object /_lakefs_actions/check_commit_metadata.yml" class="quickstart"/>
...
1. Head to the **Uncommitted Changes** tab in the UI and notice that there is now a file called `top10_lakes.parquet` waiting to be committed. 
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-03.png" alt="lakeFS branch etl_20230504 with uncommitted file top10_lakes.parquet" class="quickstart"/>
...
1. Click on **Commit Changes**, leave the _Commit message_ blank, and click **Commit Changes** to confirm. 
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-04.png" alt="lakeFS blocking an attempt to commit with no commit message" class="quickstart"/>

1. Do the same as the previous step, but provide a message this time: 

    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-05.png" alt="A commit to lakeFS with commit message in place" class="quickstart"/>
...
1. Repeat the **Commit Changes** dialog and use the **Add Metadata field** to add the required metadata: 
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-06.png" alt="A commit to lakeFS with commit message and metadata in place" class="quickstart"/>
    ...
    <img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-07.png" alt="Commit history in lakeFS showing that the commit met the rules set by the action and completed successfully." class="quickstart"/>

---

You can view the history of all action runs from the **Action** tab: 

<img width="75%" src="/api/v1/repositories/firstrepo/refs/main/objects?path=images%2Fhooks-08.png" alt="Action run history in lakeFS" class="quickstart"/>
